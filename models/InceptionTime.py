...
#来自https://github.com/Retsediv/WIFI_CSI_based_HAR/tree/master
class InceptionModel(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:  # type: ignore
        x=x.reshape(x.shape[0],x.shape[1]*x.shape[2],x.shape[3]) #(n, ntr*nsub, length)
...
def Inception(input_shape,hparams):
    input_c = input_shape[0]*input_shape[1]
    model = InceptionModel(3, input_c, input_c, 12, 15, True)
    return model
