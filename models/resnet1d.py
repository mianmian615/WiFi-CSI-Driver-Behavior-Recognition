...
def ResNet1D1111(input_shape,hparams):
    input_c=input_shape[0]*input_shape[1]
    model = ResNet1D(block=BasicBlock, layers=[1, 1, 1, 1], inchannel=input_c,if_classfier=True)
    return model

def ResNet1D18(input_shape,hparams):
    input_c=input_shape[0]*input_shape[1]
    model = ResNet1D(block=BasicBlock, layers=[2, 2, 2, 2], inchannel=input_c,if_classfier=True)
    return model

def ResNet1D34(input_shape,hparams):
    input_c=input_shape[0]*input_shape[1]
    model = ResNet1D(block=BasicBlock, layers=[3, 4, 6, 3], inchannel=input_c,if_classfier=True)
    return model
...
def ResNet1D50(input_shape,hparams):
    input_c=input_shape[0]*input_shape[1]
    model = ResNet1D(block=BasicBlock, layers=[3, 4, 6, 3], inchannel=input_c,if_classfier=True)
    return model

def ResNet1D50bott(input_shape,hparams):
    input_c=input_shape[0]*input_shape[1]
    model = ResNet1D(block=Bottleneck, layers=[3, 4, 6, 3], inchannel=input_c,if_classfier=True)
    return model
...
def ResNet1D101(input_shape,hparams):
    input_c=input_shape[0]*input_shape[1]
    model = ResNet1D(block=BasicBlock, layers=[3, 4, 23, 3], inchannel=input_c)
    return model

def ResNet1D101bott(input_shape,hparams):
    input_c=input_shape[0]*input_shape[1]
    model = ResNet1D(block=Bottleneck, layers=[3, 4, 23, 3], inchannel=input_c,if_classfier=True)
    return model
...
def ResNet1D152bott(input_shape,hparams):
    input_c=input_shape[0]*input_shape[1]
    model = ResNet1D(block=Bottleneck, layers=[3, 8, 36, 3], inchannel=input_c,if_classfier=True)
    return model
...
class URCSIResNet(torch.nn.Module):
    ...
    def __init__(self, input_shape, hparams, preserve_readout=False, freeze=None, feat_layers=None):
        ...
        self.network= ResNet1D(block=BasicBlock, layers=[1, 1, 1, 1],inchannel=input_c)
