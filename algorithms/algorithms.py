# 实现各种领域泛化算法（ERM、IRM、DANN、Mixup等）。
# 每个算法为一个类，统一接口（update、predict等）。
import torch
...
class Fish(Algorithm):
    ...
    ICLR 2022
    https://arxiv.org/abs/2104.09937
    """
    ...
    def create_clone(self, device):
        ...
        self.optimizer_inner = self.new_optimizer(self.network_inner.parameters())
...
class ARM(ERM):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        if hparams['backbone'] == 'CSIResNet':
            # CSIResNet is a special case, it has a different input shape
            self.context_net = networks.ContextNet1D(original_input_shape)
        else:
            self.context_net = networks.ContextNet2D(original_input_shape)
        self.support_size = hparams['batch_size']
        self.hparams=hparams

    def predict(self, x):
        if self.hparams['backbone'] == 'CSIResNet':
            batch_size, h, w = x.shape#torch.Size([256, 684, 1800])
        else:
        ...
        context = self.context_net(x)#torch.Size([256, 1, 1800])
        context = context.reshape((meta_batch_size, support_size, context.shape[-2],context.shape[-1]))#torch.Size([32, 8, 1, 1800])
        context = context.mean(dim=1)#torch.Size([32, 1, 1800])
        context = torch.repeat_interleave(context, repeats=support_size, dim=0)#torch.Size([256, 1,1800])
        x = torch.cat([x, context], dim=1)#torch.Size([256, 685, 1800])
        return self.network(x)
...
class SAM(ERM):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        #           = (rho / g(w).norm(2)) * g(w)
        grad_w = autograd.grad(loss, self.network.parameters(),allow_unused=True)
        new_grad_w=[]
        for g in grad_w:
            if g is None:
                break
            new_grad_w.append(g)
        scale = self.hparams["rho"] / self.norm(new_grad_w)
        eps = [g * scale for g in new_grad_w]
...
class NewAbstractDANN(Algorithm):
    """Domain-Adversarial Neural Networks (abstract class)"""

    def __init__(self, input_shape, num_classes, num_domains,
                 hparams, conditional, class_balance):

        super(NewAbstractDANN, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        ...
        # Optimizers
        self.disc_opt = self.new_optimizer(
            (list(self.discriminator.parameters()) +
                list(self.class_embeddings.parameters())))

        self.gen_opt = self.new_optimizer(
            (list(self.featurizer.parameters()) +
                list(self.classifier.parameters())))
...
class NewDANN(NewAbstractDANN):
    """Unconditional DANN"""
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(NewDANN, self).__init__(input_shape, num_classes, num_domains,
            hparams, conditional=False, class_balance=False)


class NewCDANN(NewAbstractDANN):
    """Conditional DANN"""
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(NewCDANN, self).__init__(input_shape, num_classes, num_domains,
            hparams, conditional=True, class_balance=True)
...
class RDM(ERM):
    ...
    def update(self, minibatches, unlabeled=None, held_out_minibatches=None):
        ...
        return {'update_count': self.update_count, 'total_loss': total_loss, 'erm_loss': erm_loss, 'matching_penalty': matching_penalty, 'variance_penalty': variance_penalty, 'rdm_lambda' : self.hparams['rdm_lambda']}
...
class RandomErasing(ERM):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        x=self.re(x,self.hparams["probability"],self.hparams["sl"],self.hparams["sh"],self.hparams["r1"],0)
...
class SagNet(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.optimizer_f = self.new_optimizer(self.network_f.parameters())
        self.optimizer_c = self.new_optimizer(self.network_c.parameters())
        self.optimizer_s = self.new_optimizer(self.network_s.parameters())
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        return {'loss_c': loss_c, 'loss_s': loss_s, 'loss_adv': loss_adv}
...
class SD(ERM):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        return {'loss': loss, 'penalty': penalty}
...
class IGA(ERM):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        return {'loss': mean_loss, 'penalty': penalty_value}
...
class Fishr(Algorithm):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        return {'loss': objective, 'nll': all_nll, 'penalty': penalty}
...
class TRM(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.optimizer_c = self.new_optimizer(
...
class IBERM(ERM):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        self.update_count += 1
        return {'loss': loss,'nll': nll, 'IB_penalty': ib_penalty}
...
class IBIRM(ERM):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        self.update_count += 1
        return {'loss': loss,'nll': nll, 'IRM_penalty': irm_penalty, 'IB_penalty': ib_penalty}
...
class AbstractCAD(Algorithm):
    ...
    def update(self, minibatches, unlabeled=None):  
        ...
        return {"clf_loss": clf_loss, "bn_loss": bn_loss, "total_loss": total_loss}
...
class ADRMX(Algorithm):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        if (self.update_count.item() % (1+d_steps_per_g) < d_steps_per_g):
            ...
            return {'loss_total': gen_loss, 
                'loss_cnt_int': cnt_loss_int,
                'loss_cnt_final': cnt_loss_final,
                'loss_clf_int': classifier_loss_int, 
                'loss_clf_fin': classifier_loss_final, 
                'loss_dmn': classifier_loss_domain, 
                'loss_disc': disc_loss,
                'loss_remixed': classifier_remixed_loss,
                }
...
class LIRR(ERM):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams, conditional=False, class_balance=False):
        ...
        self.env_classifier = networks.Classifier(self.featurizer.n_outputs + 1, num_classes,
                                                  self.hparams)
        self.domain_indx = [torch.full((int(hparams['batch_size']/num_domains), 1), indx) for indx in range(num_domains)]
        self.discriminator = networks.MLP(self.featurizer.n_outputs,
                                          num_domains, self.hparams)
        self.class_embeddings = nn.Embedding(num_classes,
                                             self.featurizer.n_outputs)
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        if (self.update_count.item() % (1 + d_steps_per_g) < d_steps_per_g):
            ...
            a=torch.cat([all_z, embeddings], 1)
...
class IIB(ERM):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        total_loss.backward()
        self.optimizer.step()
        return {'loss_ib': ib_loss, 'loss_env': env_loss, 'loss_inv': inv_loss,
                'loss_all': total_loss}
